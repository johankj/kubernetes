...
func printEvictionRequest(obj *coordination.EvictionRequest, options printers.GenerateOptions) ([]metav1.TableRow, error) {
	...
	if len(lastActiveInterceptorName) > 0 {
		for _, interceptor := range obj.Status.Interceptors {
			...
			if interceptor.CompletionTime != nil {
			...
			} else if isFailed && (failed.Reason == string(coordination.EvictionRequestConditionReasonCanceledDueToNoRequesters) ||
				failed.Reason == string(coordination.EvictionRequestConditionReasonNoFurtherInterceptor)) {
				...
				if interceptor.ExpectedCompletionTime != nil {
					if interceptor.ExpectedCompletionTime.After(time.Now()) {
						...
						activeInterceptorExpectedCompletionTime = fmt.Sprintf("%s ago", translateTimestampSince(*interceptor.ExpectedCompletionTime))
					}
				}
			}
			if options.Wide {
