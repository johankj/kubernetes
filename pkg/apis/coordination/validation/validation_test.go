...
func TestValidateEvictionRequestStatusUpdate(t *testing.T) {
	...
	successCases := map[string]struct {
		...
		"processedInterceptors must allow active interceptor transition even with an exceeded deadline (start time fallback) - Failed": {
			oldInput: mkValidEvictionRequestStatus(1,
				addActiveInterceptorsCount(1),
		...
		"processedInterceptors must allow active interceptor transition even with an exceeded deadline - Failed": {
			oldInput: mkValidEvictionRequestStatus(1,
				addActiveInterceptorsCount(1),
				setInterceptorsStartTime(clockBefore(20*time.Minute), 0, 1),
	...
	failureCases := map[string]struct {
		...
		"no new activeInterceptors are allowed when the eviction request has failed status": {
			oldInput: mkValidEvictionRequestStatus(1),
			input: mkValidEvictionRequestStatus(1,
				addActiveInterceptors(interceptorName(0)),
				addCondition(clock, coordination.EvictionRequestConditionFailed, true)),
		...
		"status interceptors cannot mutate name when active - short circuited by targetInterceptors key order": {
			oldInput: mkValidEvictionRequestStatus(2, addActiveInterceptorsCount(1)),
			input: mkValidEvictionRequestStatusWithStatuses(2, 0,
				addActiveInterceptorsCount(1),
				addInterceptorStatusesNames(interceptorName(1), interceptorName(0))),
			errors: []*field.Error{
		...
		"startTime cannot be removed once set": {
			...
			input: mkValidEvictionRequestStatus(2,
				addActiveInterceptorsCount(1)),
			errors: []*field.Error{
				field.Invalid(field.NewPath("status", "interceptors").Index(0).Child("startTime"), nil, "field cannot be cleared once set").MarkCoveredByDeclarative(),
			},
		},
		"startTime cannot be changed once set": {
			...
			errors: []*field.Error{
				field.Invalid(field.NewPath("status", "interceptors").Index(0).Child("startTime"), nil, "field cannot be modified once set").MarkCoveredByDeclarative(),
			},
		},
		...
		"completionTime cannot be changed once set": {
			...
			errors: []*field.Error{
				field.Invalid(field.NewPath("status", "interceptors").Index(0).Child("completionTime"), nil, "field cannot be modified once set").MarkCoveredByDeclarative(),
			},
		},
		"completionTime cannot be removed once set": {
			...
			input: mkValidEvictionRequestStatus(2,
				...
				setInterceptorsStartTime(clock, 0, 1)),
			errors: []*field.Error{
				field.Invalid(field.NewPath("status", "interceptors").Index(0).Child("completionTime"), nil, "field cannot be cleared once set").MarkCoveredByDeclarative(),
			},
		},
		...
		// message
		// Truncated in the strategy (see TestEvictionRequestStatusStrategy) instead of strict validation.
		...
		"too long message is truncated": {
			...
			errors: []*field.Error{
				field.TooLongCharacters(field.NewPath("status", "interceptors").Index(0).Child("message"), "", 4000).WithOrigin("maxLength").MarkCoveredByDeclarative(),
			},
		},
		// conditions
		"too many conditions": {
		...
		"Failed condition cannot be removed": {
			...
			input:    mkValidEvictionRequestStatus(1),
			errors: []*field.Error{
				field.Invalid(field.NewPath("status", "conditions"), "", "Failed condition is immutable"),
			},
		},
...
func mkValidEvictionRequestStatusWithStatuses(interceptors, statuses int, tweaks ...func(obj *coordination.EvictionRequestStatus)) *coordination.EvictionRequestStatus {
	...
	for i := range interceptors {
		obj.TargetInterceptors = append(obj.TargetInterceptors, coordination.TargetInterceptor{
...
func addTargetInterceptors(interceptors ...string) func(obj *coordination.EvictionRequestStatus) {
	return func(obj *coordination.EvictionRequestStatus) {
		for _, interceptorName := range interceptors {
			obj.TargetInterceptors = append(obj.TargetInterceptors, coordination.TargetInterceptor{Name: interceptorName})
		}
	}
}
