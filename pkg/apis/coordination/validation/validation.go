...
func ValidateAllEvictionRequestStatusInterceptorFields(status, oldStatus *coordination.EvictionRequestStatus, fldPath *field.Path, opts EvictionRequestStatusInterceptorsValidationOptions) field.ErrorList {
	...
	// more than .pod.spec.evictionInterceptors to account for default interceptors
...
func targetInterceptorsToNames(targetInterceptors []coordination.TargetInterceptor) []string {
...
func ValidateEvictionRequestTargetInterceptors(targetInterceptors, oldTargetInterceptors []coordination.TargetInterceptor, fldPath *field.Path, opts ValidateEvictionRequestTargetInterceptorsOptions) field.ErrorList {
	var allErrs field.ErrorList
	...
	for i, interceptor := range targetInterceptors {
		allErrs = append(allErrs, ValidateTargetInterceptor(interceptor, fldPath.Index(i))...)
	}
	return allErrs
}

func ValidateTargetInterceptor(evictionInterceptor coordination.TargetInterceptor, fldPath *field.Path) field.ErrorList {
	var allErrs field.ErrorList
	...
	if len(evictionInterceptor.Name) == 0 {
		...
		allErrs = append(allErrs, apivalidation.ValidateEvictionRequestParticipantName(namePath, evictionInterceptor.Name, nil)...)
	}
	return allErrs
}
...
func ValidateEvictionRequestActiveInterceptors(activeInterceptors, oldActiveInterceptors []string, fldPath *field.Path, opts ValidateEvictionRequestActiveInterceptorsOptions) field.ErrorList {
	...
	if activeInterceptorsSet.Difference(oldActiveInterceptorsSet).Len() > 0 && (opts.IsEvicted || opts.IsFailed) {
...
func ValidateEvictionRequestStatusInterceptor(status, oldStatus *coordination.InterceptorStatus, fldPath *field.Path, opts EvictionRequestStatusInterceptorValidationOptions) field.ErrorList {
	...
	// message
	// Truncated in the strategy instead of validation.
...
func ValidateEvictionRequestProcessedInterceptors(processedInterceptors, oldProcessedInterceptors []string, fldPath *field.Path, opts ValidateEvictionRequestProcessedInterceptorsOptions) field.ErrorList {
	...
	for i, interceptor := range processedInterceptors {
		...
		if interceptorStatus.CompletionTime == nil && !opts.IsEvicted && !opts.IsFailed {
